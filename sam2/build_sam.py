...
def build_sam2_tomogram_predictor(
    config_file,
    ckpt_path=None,
    device="cuda",
    mode="eval",
    ...
    **kwargs,
    ...
    hydra_overrides = [
        "++model._target_=sam2.sam2_tomogram_predictor.SAM2TomogramPredictor",
    ]
    if apply_postprocessing:
        hydra_overrides_extra = hydra_overrides_extra.copy()
        hydra_overrides_extra += [
...
def build_sam2_tomogram_predictor_hf(model_id, **kwargs):
    ...
    config_name, checkpoint_name = model_id_to_filenames[model_id]
    ckpt_path = hf_hub_download(repo_id=model_id, filename=checkpoint_name)
    return build_sam2_tomogram_predictor(
        config_file=config_name, ckpt_path=ckpt_path, **kwargs
    )
